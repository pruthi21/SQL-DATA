...
select * from bill natural join pharmacy;
select * from patient natural join nurse;
select * from patient natural join staff;
show tables;
...
select * from patient right join room on room.r_type=patient.name ;
...
select * from bill where b_id =(select max(b_id) from bill);
select * from bill where b_id =(select max(b_id) from bill where b_id < (select max(b_id) from bill where b_id < (select max(b_id) from bill)));
alter table patient  add column blood_group varchar(20)after age ;
desc patient;
insert into patient(blood_group) values('o','o','ab','b','a');
